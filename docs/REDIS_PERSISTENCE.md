...
`GET /management/status` endpoint — a `"config_persisted": false` response
...
|-------|--------|---------|
| `redis_persistence` | `true` / `false` / `"unknown"` | Whether AOF is currently enabled in Redis |
| `config_persisted` | `true` / `false` / `null` | Whether CONFIG REWRITE succeeded at startup. `null` means persistence was not attempted (`TAS_REDIS_PERSISTENCE=false`). |

**Action required when `config_persisted` is `false`:**
