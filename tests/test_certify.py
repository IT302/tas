...
from tests.cert_test_utils import API_HEADERS, build_certify_payload, get_nonce
from tests.test_csr_sanitization import generate_csr
...
def test_certify_emits_canonical_evidence_digests(test_client, monkeypatch):
    ...
    assert evidence_doc == {
        ...
        "entries": [
            {
                "digest": hashlib.sha256(cpu_raw).hexdigest(),
                "digest_alg": "sha-256",
            ...
            {
                "digest": hashlib.sha256(gpu0_raw).hexdigest(),
                "digest_alg": "sha-256",
            ...
            {
                "digest": hashlib.sha256(gpu1_raw).hexdigest(),
                "digest_alg": "sha-256",
                "evidence_type": "gpu-attestation-report",
                "platform_id": "nvidia-h100",
                "slot": 1,
